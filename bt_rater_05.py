...
df = pd.read_csv("bt_batch_05.csv", encoding="utf-8-sig")
df.columns = [col.strip() for col in df.columns]  # 去除列名首尾空格

st.set_page_config(page_title="Backtranslation Rating - Rater 05", layout="centered")
st.title("📝 Simplification Back-Translation Evaluation - Rater 05")
...
if st.button("💾 Export Ratings to CSV"):
    pd.DataFrame(ratings).to_csv(f"bt_ratings_rater05.csv", index=False, encoding="utf-8-sig")
